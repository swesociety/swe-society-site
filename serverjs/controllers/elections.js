...
const createCommitteeMember = errorWrapper(
  async (req, res) => {
    ...
    if (existingMember.length > 0) {
      throw new CustomError("This user already has a role in this executive committee", 409);
    }
  ...
  async (req, res) => {
    const { rows } = await pool.query(`
      SELECT
        ...
        u.profile_picture,
        cp.post_name,
        ec.committee_name AS executive_committee_name,
        ec.year AS executive_committee_year,
        e.year,
        e.election_type,
        e.batch
      FROM Committee c
...
const updateCommitteeMember = errorWrapper(
  async (req, res) => {
    ...
    const { rows: existingMember } = await pool.query(
      ...
      [userid, executive_committeeid, committeeid]
    );
    if (existingMember.length > 0) {
      throw new CustomError("This user already has a role in this executive committee", 409);
    }
...
//------------- standalone executive committees ----------------------

const createExecutiveCommittee = errorWrapper(
  async (req, res) => {
    const { committee_name, year } = req.body;
    if (!committee_name || !year) {
      throw new CustomError("Committee name and year are required", 400);
    }
    const { rows } = await pool.query(
...
const updateExecutiveCommittee = errorWrapper(
  async (req, res) => {
    ...
    if (rows.length === 0) {
      throw new CustomError("Executive committee not found", 404);
    }
    res.json(rows[0]);
  },
...
const deleteExecutiveCommittee = errorWrapper(
  async (req, res) => {
    const { rowCount } = await pool.query(
      "DELETE FROM ExecutiveCommittees WHERE committeeid = $1",
