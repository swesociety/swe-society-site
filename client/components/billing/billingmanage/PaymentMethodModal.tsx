...
const PaymentMethodModal: React.FC<Props> = ({ onClose }) => {
  const [methods, setMethods] = useState<PaymentMethod[]>([]);
  const [editIndex, setEditIndex] = useState<number | null>(null);
  const [newMethod, setNewMethod] = useState<Partial<PaymentMethod> | null>(null);
...
const handleDelete = async (index: number) => {
    ...
    if (!method) return;
    try {
        await axios.delete(`${APIENDPOINTS.billing.deleteMethodType}/${method.payment_methodid}`, headerConfig());
  ...
  const handleEditSubmit = async (id: number) => {
    try {
      await axios.put(`${APIENDPOINTS.billing.updateMethodType}/${id}`, editData, headerConfig());
      setMethods((prev) =>
        prev.map((method) =>
  ...
  const handleAddNewMethod = async () => {
    ...
    try {
      const res = await axios.post(`${APIENDPOINTS.billing.createMethodType}`, newMethod, headerConfig());
          ...
          <table className="w-full text-left border-collapse border text-sm">
            ...
            <tbody>
              {methods.map((method, index) => (
                <tr key={method.payment_methodid} className="border-b border-gray-700">
                  {editIndex === index ? (
                    <>
                      <td className="p-2">
                        <Input
                      ...
                      <td className="p-2">
                        <Input
                          ...
                          onChange={(e) =>
                            handleEditChange("transaction_account", e.target.value)
                          }
                          className="bg-gray-800 text-white"
                        />
                      </td>
                      ...
                      <td className="p-2">{new Date(method.created_at).toLocaleDateString()}</td>
                      <td className="p-2 flex gap-2 justify-end">
                        <Check
                          className="text-green-500 cursor-pointer"
                          onClick={() => handleEditSubmit(method.payment_methodid)}
                        />
                        <X
            ...
            <div className="mt-4 space-y-2 border-t border-gray-700 pt-4">
              <div className="flex gap-2 p-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNewMethod({ ...newMethod, transaction_account: e.target.value })
                  }
                  className="bg-gray-800 text-white"
                />
                <Input
